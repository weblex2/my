...
<head>
    ...
    <style>
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .trip-name  { font-size: 13px; font-weight: 600; color: #f1f5f9; line-height: 1.2; }
        .trip-code  { font-size: 11px; color: #475569; margin-top: 1px; }
        ...
        .close-btn:hover { background: rgba(255,255,255,0.09); color: #e2e8f0; }
        ...
        .photo-card:hover { border-color: rgba(249,115,22,0.25); }

        .photo-card .pic-wrap { position: relative; cursor: pointer; }
        ...
        .photo-card video { width: 100%; display: block; }
        ...
        .photo-date a { color: #f97316; text-decoration: none; font-size: 11px; }
        ...
        .alt-card:hover { background: rgba(255,255,255,0.04); }
        .alt-card .alt-name { font-size: 13px; font-weight: 600; color: #e2e8f0; }
        .alt-card .alt-code { font-size: 11px; color: #475569; }
...
am5.ready(function () {
    ...
    pinSeries.bullets.push(function () {
        var cont = am5.Container.new(root, { cursorOverStyle: "pointer" });
    ...
    fetch('/travel-blog/map-data/' + id)
        ...
        .then(function(data) {
            ...
            if (mps.length > 1) {
                ...
                var allCoords = mps.map(function(mp) { return [mp.lon, mp.lat]; });
            ...
            m.chart.zoomToGeoPoint(
                { longitude: mps[0].lon, latitude: mps[0].lat },
                2.5, true
            );
        });
}
...
function renderPhotoCard(photo) {
    ...
    var clickAttr = photo.is_video ? '' : ' onclick="showFullPic(' + photo.id + ')"';
...
function renderAlternatives(alts) {
    ...
    alts.forEach(function(g) {
        list.insertAdjacentHTML('beforeend',
            '<div class="alt-card" onclick="loadGallery(' + g.id + ', \'' + g.name.replace(/'/g,"\'") + '\', \'' + g.code + '\'); closePanel();">' +
